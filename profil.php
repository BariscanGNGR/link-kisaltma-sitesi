      ...
      <div class="col-md-7 col-lg-12">
        ...
        <form class="needs-validation" method="post" action="" novalidate>
            ...
            <?php 
                ...
                $id = kullanici_id($sql,$_SESSION['kullanici']);
                
                $sorgu = mysqli_query($sql,"SELECT `id`, `link`, `kisalt`, `uye_id` FROM `linkler` WHERE `uye_id`='$id';") or die(mysqli_error($sql));
                ...
                for($i = 0 ; $i < $satirSayi ; $i++)
                {
                 ...
                 <a class="link-secondary" href="'.$satir['link'].'">'.$satir['link'].'</a><br>
                 <a class="link-success" href="'.$satir['kisalt'].'">'.$satir['kisalt'].'</a><br>
                 <button name="duzenle" value="'.$satir['id'].'" class="btn btn-sm btn-secondary">Düzenle</button>
                 <button name="sil" value="'.$satir['id'].'" class="btn btn-sm btn-danger">Sil</button>
                 <br><br>
                 </div>';
            ...
            <?php
                ...
                if(isset($_POST['sil']))
                {
                  ...
                  $sorgu2 = mysqli_query($sql,"SELECT `link`, `kisalt`,`klasor` FROM `linkler` WHERE `id`='$id'") or die(mysqli_error($sql));;
                  $satir2 = mysqli_fetch_array($sorgu2);

                  unlink("./".$satir2['klasor']."/index.html");
                  rmdir("./".$satir2['klasor']."/");

                  $sorgu = mysqli_query($sql,"DELETE FROM `linkler` WHERE `id`='$id'") or die(mysqli_error($sql));

                  header("Refresh:0");
                }
                if(isset($_POST['duzenle']))
                {
                    header('location:duzenle.php?id='.$_POST['duzenle']);
                }
            ?>
